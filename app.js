...
// Load data from Cloud Storage (OneDrive, Google Drive, etc.)
async function loadFromGoogleSheets() {
    // Check if OneDrive URLs are configured
    const useOneDrive = GOOGLE_SHEETS_CONFIG && (
        GOOGLE_SHEETS_CONFIG.orderFileUrl ||
        GOOGLE_SHEETS_CONFIG.loadingFileUrl ||
        GOOGLE_SHEETS_CONFIG.packingFileUrl
    );

    if (useOneDrive) {
        return loadFromOneDrive();
    }

    // Fallback to Google Sheets method
    return loadFromGoogleSheetsOld();
}

// Load from Google Sheets directly (no backend needed!)
async function loadFromOneDrive() {
    const orderUrl = GOOGLE_SHEETS_CONFIG.orderFileUrl || document.getElementById('orderSheetUrl')?.value.trim();
    const loadUrl = GOOGLE_SHEETS_CONFIG.loadingFileUrl || document.getElementById('loadSheetUrl')?.value.trim();
    const packUrl = GOOGLE_SHEETS_CONFIG.packingFileUrl || document.getElementById('packSheetUrl')?.value.trim();

    if (!orderUrl || !loadUrl || !packUrl) {
        alert('Please configure Google Sheets URLs in sheets-config.js\n\nSee GOOGLE_DRIVE_SETUP.md for instructions.');
        return;
    }
    ...
    <div style="text-align:center;padding:40px;">
      ...
      <p>Please wait while we fetch your Excel files.</p>
  ...
  `;
    ...
    try {
        console.log('Fetching from Google Sheets:', { orderUrl, loadUrl, packUrl });

        // Helper function to convert Google Sheets URL to export URL
        function convertToExportUrl(url) {
            ...
            // If already in export format, return as is
        ...
        // Convert URLs to export format
        const orderExportUrl = convertToExportUrl(orderUrl);
        const loadExportUrl = convertToExportUrl(loadUrl);
        const packExportUrl = convertToExportUrl(packUrl);

        console.log('Export URLs:', { orderExportUrl, loadExportUrl, packExportUrl });

        // Fetch all three Excel files directly from Google Sheets
        const [orderResponse, loadResponse, packResponse] = await Promise.all([
        ...
        // Check responses
        if (!orderResponse.ok) {
            throw new Error(`Failed to fetch Order file (${orderResponse.status}). Make sure the sheet is publicly accessible.`);
        }
        if (!loadResponse.ok) {
            throw new Error(`Failed to fetch Loading file (${loadResponse.status}). Make sure the sheet is publicly accessible.`);
        }
        if (!packResponse.ok) {
            throw new Error(`Failed to fetch Packing file (${packResponse.status}). Make sure the sheet is publicly accessible.`);
        }

        // Get array buffers
        const orderBuffer = await orderResponse.arrayBuffer();
        const loadBuffer = await loadResponse.arrayBuffer();
        const packBuffer = await packResponse.arrayBuffer();

        console.log('Files downloaded, parsing Excel...');

        // Parse Excel files using XLSX library
        const orderWorkbook = XLSX.read(orderBuffer, { type: 'array' });
        const loadWorkbook = XLSX.read(loadBuffer, { type: 'array' });
        const packWorkbook = XLSX.read(packBuffer, { type: 'array' });
        ...
        const packSheet = packWorkbook.Sheets[packWorkbook.SheetNames[0]];

        // Convert to 2D array (same format as readExcel function)
        const orderRaw = XLSX.utils.sheet_to_json(orderSheet, { header: 1 });
        const loadRaw = XLSX.utils.sheet_to_json(loadSheet, { header: 1 });
        const packRaw = XLSX.utils.sheet_to_json(packSheet, { header: 1 });

        console.log('Parsed rows:', {
            order: orderRaw.length,
            load: loadRaw.length,
            pack: packRaw.length
        });

        // Process data using existing normalize and merge functions
        const orders = normalize(orderRaw, "ORDER");
        const loads = normalize(loadRaw, "LOAD");
        const packs = normalize(packRaw, "PACK");
        ...
        output.innerHTML = `
      <div style="background:#f8d7da;color:#721c24;padding:30px;border-radius:8px;border-left:4px solid #dc3545;margin:20px;max-width:800px;">
        ...
        <div style="background:#fff3cd;color:#856404;padding:15px;border-radius:5px;margin:20px 0;">
          <h4 style="margin-top:0;">📋 Google Sheets Setup:</h4>
          <p><strong>Make sure your sheets are publicly accessible:</strong></p>
          <ol style="line-height:1.8;">
            <li>Open each Google Sheet</li>
            <li>Click "Share" button</li>
            <li>Set to "Anyone with the link can view"</li>
            <li>Copy the share link</li>
            <li>Update sheets-config.js with the URLs</li>
          </ol>
          <p><strong>See GOOGLE_DRIVE_SETUP.md for detailed instructions</strong></p>
        </div>

        <div style="background:#e7f3ff;color:#004085;padding:15px;border-radius:5px;margin:20px 0;">
          <h4 style="margin-top:0;">🔧 Alternative: Use File Upload</h4>
          <p>Click the "Upload Files" radio button above and upload your Excel files directly.</p>
        </div>

        <p><strong>Current URLs:</strong></p>
        <ul style="font-size:12px;word-break:break-all;">
          <li>Order: ${orderUrl}</li>
          <li>Loading: ${loadUrl}</li>
          <li>Packing: ${packUrl}</li>
        </ul>
      </div>
    `;
    }
...
// Old Google Sheets method (CSV-based)
async function loadFromGoogleSheetsOld() {
    const orderUrl = document.getElementById('orderSheetUrl').value.trim();
    const loadUrl = document.getElementById('loadSheetUrl').value.trim();
    const packUrl = document.getElementById('packSheetUrl').value.trim();

    if (!orderUrl || !loadUrl || !packUrl) {
        alert('Please enter all three Google Sheets URLs');
        return;
    }
  ...
  `;
    ...
    try {
        // Convert Google Sheets share URL to published CSV URL
        const getPublishedCsvUrl = (url) => {
            if (!url) return null;
            ...
            // Use published CSV format which doesn't have CORS issues
            return `https://docs.google.com/spreadsheets/d/${sheetId}/export?format=csv&gid=0`;
        };

        const orderCsvUrl = getPublishedCsvUrl(orderUrl);
        const loadCsvUrl = getPublishedCsvUrl(loadUrl);
        const packCsvUrl = getPublishedCsvUrl(packUrl);

        console.log('Fetching from:', { orderCsvUrl, loadCsvUrl, packCsvUrl });

        // Fetch all three CSV files
        const [orderResponse, loadResponse, packResponse] = await Promise.all([
            fetch(orderCsvUrl),
            fetch(loadCsvUrl),
            fetch(packCsvUrl)
        ...
        // Check responses
        if (!orderResponse.ok) {
            throw new Error(`Failed to fetch Order sheet (${orderResponse.status}). Make sure the sheet is published to web.`);
        }
        if (!loadResponse.ok) {
            throw new Error(`Failed to fetch Loading sheet (${loadResponse.status}). Make sure the sheet is published to web.`);
        }
        if (!packResponse.ok) {
            throw new Error(`Failed to fetch Packing sheet (${packResponse.status}). Make sure the sheet is published to web.`);
        }

        // Get CSV text
        const orderCsv = await orderResponse.text();
        const loadCsv = await loadResponse.text();
        const packCsv = await packResponse.text();

        console.log('CSV data received, parsing...');

        // Parse CSV to array format (same as Excel)
        const parseCSV = (csv) => {
        ...
        const orderRaw = parseCSV(orderCsv);
        const loadRaw = parseCSV(loadCsv);
        const packRaw = parseCSV(packCsv);
      ...
      <div style="background:#f8d7da;color:#721c24;padding:30px;border-radius:8px;border-left:4px solid #dc3545;margin:20px;max-width:800px;">
        ...
        <div style="background:#fff3cd;color:#856404;padding:15px;border-radius:5px;margin:20px 0;">
          ...
          <p><strong>You MUST publish your sheets to the web:</strong></p>
          <ol style="line-height:1.8;">
            <li>Open your Google Sheet</li>
            <li>Click <strong>File → Share → Publish to web</strong></li>
            <li>Select <strong>"Entire Document"</strong> or specific sheet</li>
            <li>Choose <strong>"Comma-separated values (.csv)"</strong></li>
            <li>Click <strong>"Publish"</strong></li>
            <li>Click <strong>"OK"</strong> to confirm</li>
            <li>Copy the published link (or use your share link)</li>
          </ol>
        </div>

        <div style="background:#e7f3ff;color:#004085;padding:15px;border-radius:5px;margin:20px 0;">
          <h4 style="margin-top:0;">🔧 Alternative: Run Local Server</h4>
          <p>If publishing doesn't work, run a local web server:</p>
          <code style="background:#fff;padding:10px;display:block;border-radius:4px;margin:10px 0;">
            python3 -m http.server 8000
          </code>
          <p>Then open: <code>http://localhost:8000/order-load-pack-hide-zero-load.html</code></p>
        </div>

        <p><strong>Current URLs being used:</strong></p>
        <ul style="font-size:12px;word-break:break-all;">
          <li>Order: ${orderUrl}</li>
          <li>Loading: ${loadUrl}</li>
          <li>Packing: ${packUrl}</li>
...
// Load URLs from config on page load
window.addEventListener('DOMContentLoaded', () => {
    const orderInput = document.getElementById('orderSheetUrl');
    const loadInput = document.getElementById('loadSheetUrl');
    const packInput = document.getElementById('packSheetUrl');

    if (orderInput && typeof GOOGLE_SHEETS_CONFIG !== 'undefined') {
        orderInput.value = GOOGLE_SHEETS_CONFIG.orderSheetUrl || '';
    }
    if (loadInput && typeof GOOGLE_SHEETS_CONFIG !== 'undefined') {
        loadInput.value = GOOGLE_SHEETS_CONFIG.loadingSheetUrl || '';
    }
    if (packInput && typeof GOOGLE_SHEETS_CONFIG !== 'undefined') {
        packInput.value = GOOGLE_SHEETS_CONFIG.packingSheetUrl || '';
    }
});

// Toggle between Google Sheets and File Upload
